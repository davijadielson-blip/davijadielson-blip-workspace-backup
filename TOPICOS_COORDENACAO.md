# TOPICOS_COORDENACAO.md - Controle de tópicos sob coordenação do Jarvis

Grupo: LÓGIKA (`telegram:-1003645702069`)
Responsável de coordenação/supervisão: Jarvis
Modelo organizacional: empresa por departamentos + frentes/agentes por missão.

## Status geral

- Roteamento dos tópicos: corrigido/visível para Jarvis em 2026-06-05.
- Tópico **5 — Starter Kit / Sistema**: removido/desconsiderado por orientação de Jahdielson. É assunto do sistema, não deve permanecer dentro da operação LÓGIKA por enquanto.
- Jarvis deve coordenar, treinar e supervisionar; cada agente/tópico receberá missão específica de Jahdielson no privado.

## Mapa dos tópicos sob coordenação ativa

| Tópico | Nome | Departamento-base | Papel do Jarvis | Status |
|---:|---|---|---|---|
| 1 | LÓGIKA / Coordenação geral | Direção / Estratégia | Coordenação geral da operação | ativo |
| 6 | Secretária / Agenda / Execução | Operações Internas / Administrativo | Supervisão de rotina, agenda e execução | ativo |
| 8 | Lógika Creative | Produção / Entregas + Marketing / Conteúdo | Supervisão criativa e entregas visuais/conceituais | ativo |
| 9 | Saúde São Sebastião | Atendimento / Clientes + Produção / Entregas | Supervisão separada de cliente/projeto | ativo |
| 10 | Câmara Municipal | Atendimento / Clientes + Produção / Entregas | Supervisão separada de cliente/projeto | ativo |
| 11 | SINDSS | Atendimento / Clientes + Produção / Entregas | Supervisão separada de cliente/projeto | ativo |
| 12 | Produção / Técnica / Equipamentos | Produção / Entregas | Supervisão operacional/técnica e recursos | ativo |
| 13 | Comercial / Prospecção / Propostas | Comercial / Propostas | Supervisão comercial, funil e propostas | ativo |
| 14 | Laboratório / Testes | Operações Internas / P&D | Supervisão de testes; escalar arquitetura/sistema para Lôh | ativo |
| 96 | MY FINANCE | Financeiro / Administrativo | Supervisão financeira/administrativa do tema | ativo |
| 474 | Referências / Inspirações | Estratégia + Marketing / Conteúdo | Curadoria, repertório e inteligência criativa | ativo |
| 550 | Marketing | Marketing / Conteúdo | Supervisão de marca, campanhas e comunicação | ativo |
| 551 | Estrategista | Direção / Estratégia | Supervisão estratégica e priorização | ativo |
| 552 | Produção de Conteúdo | Marketing / Conteúdo + Produção / Entregas | Supervisão de pauta, produção e cadência | ativo |

## Tópicos desconsiderados por enquanto

| Tópico | Nome | Motivo |
|---:|---|---|
| 5 | Starter Kit / Sistema | Jahdielson informou que é questão do sistema e será retirado da LÓGIKA por enquanto. |

## Diretriz-base para todos os tópicos

Jarvis se apresenta como coordenação/supervisão operacional da LÓGIKA. O tópico funciona como frente/departamento independente ligado ao título. Jarvis orienta escopo, organização, próximos passos, pendências, limites e segurança; não assume a missão principal do agente. Detalhes específicos serão alinhados por Jahdielson no privado.
