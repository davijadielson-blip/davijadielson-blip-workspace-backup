---
tema: classificação de arquivos não markdown no Cofre
conteudo: classificação por risco e destino provável de arquivos não-.md, sem expor conteúdo
nicho: ecossistema agêntico Lôh/Jadielson
setor: governança do Cofre
cliente: Jadielson Davi
tipo: inventário/classificação
prioridade: alta
atualizado_em: 2026-07-26
usar_quando: decidir próximos movimentos seguros para arquivos que não obedecem ao padrão .md do Cofre
nao_usar_quando: expor conteúdo de arquivos sensíveis; este relatório lista caminhos apenas
---

# Classificação de arquivos não-.md

Total: 239

## Diretriz
- Não apagar.
- Não abrir/expor sensíveis.
- Binários e documentos devem ir para Google Drive ou arquivo externo, mantendo apenas resumo `.md` no Cofre.
- Técnicos necessários ao funcionamento podem permanecer temporariamente até decisão arquitetural, mas devem ser isolados de memória consultável por agentes.

## 01-sensiveis-nao-expor — 11
- `.env`
...
## 02-binarios-candidatos-drive — 14
...
## 04-tecnicos-scripts-config — 140
- `.gitignore`
- `Dockerfile`
- `tmp_append_pneu.py`
