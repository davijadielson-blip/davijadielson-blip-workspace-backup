...
Liste os arquivos criados em `memory/logs/comandos/` nos últimos 7 dias:
```bash
find "memory/logs/comandos" -name "*.md" -newer $(date -v-7d +"%Y-%m-%d") 2>/dev/null | sort
```
...
Encontre todos os arquivos em `memory/outputs/` com `revisado: false` criados há mais de 7 dias:
```bash
find "memory/outputs" -name "*.md" -not -newer $(date -v-7d +"%Y-%m-%d") 2>/dev/null | sort
```
...
3a. **Notas órfãs em memory/outputs/** — arquivos sem nenhum link de entrada:
```bash
...
3b. **Pasta Inbox** — verifique se há itens em `10-pessoal/inbox/` há mais de 3 dias:
```bash
find "10-pessoal/inbox" -name "*.md" -not -newer $(date -v-3d +"%Y-%m-%d") 2>/dev/null
```
Se encontrar, liste e sugira destino baseado no conteúdo.

3c. **Agentes desatualizados** — liste os arquivos em `memory/agents/` com data de criação (não checagem de atualização automática — apenas informe a data do campo `data-criacao` de cada um para Jadielson avaliar se precisam de revisão).

**PASSO 4 — Gerar relatório**

Crie `memory/logs/manutencao/YYYY-MM-DD.md`:
