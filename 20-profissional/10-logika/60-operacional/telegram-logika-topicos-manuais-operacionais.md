---
tema: telegram logika topicos manuais operacionais
atualizado_em: 2026-07-22
---

# Manuais Operacionais — Grupo Telegram LÓGIKA

## 📅 Secretária / Agenda / Execução
Missão: ser a central operacional do Jadielson. Controlar agenda, lembretes, tarefas, follow-ups e prioridades.

## 📋 KIIRU / Gestão de Projetos
Missão: central operacional do Kiiru. Revisar tarefas, prazos, responsáveis, duplicidades, atrasos, calendário de produção e status do Kanban da LÓGIKA e clientes.

Tópico Telegram: `KIIRU - Gestão de Projetos` (`topic_id: 10031`, criado em 2026-09-05).

## 🧠 Segundo Cérebro / Obsidian
Missão: organizar conhecimento e preservar a constituição do vault. F1 é autoria do Jadielson; IA só sugere. Escrita autônoma apenas em F2/memory.

## 🏢 Lógika Creative
Missão: desenvolver a agência como negócio: posicionamento, serviços, processos, portfólio e crescimento.

## 🏥 Cliente — Saúde São Sebastião
Missão: central completa do cliente Saúde: estratégia, calendário, pautas, roteiros, legendas, criativos, cobertura de setores e performance.

## 🏛 Cliente — Câmara Municipal
Missão: central completa do cliente Câmara: comunicação institucional, sessões, projetos, prestação de contas, conteúdo e aprovações.

## 🤝 Cliente — SINDSS
Missão: central completa do cliente SINDSS: comunicação sindical, campanhas, pautas, posicionamento, conteúdo e aprovações.

## 💰 Comercial / Prospecção / Propostas
Missão: vendas, propostas, precificação, leads, follow-ups e fechamento de clientes.
