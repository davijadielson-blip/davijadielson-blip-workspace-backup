#!/usr/bin/env python3
"""Migra itens do DS antigo para o banco classico novo."""
...
cursor = None; batch = 0; migrated = 0; berrs = 0
...
print("\n"+"="*60, flush=True)
print(f"MIGRADO: {migrated}\nErros: {berrs}\nURL: https://app.notion.com/p/{NEW_DB_ID}\n", flush=True)

vr, vc, verr = api('POST', '/databases/'+NEW_DB_ID+'/query', {'page_size': 10})
if vr and vc == 200:
    vt = vr.get('total','?')
    ...
    for ix, it in enumerate(vr.get('results',[])[:10]):
        ...
        if f: print(f"     Frente: {f}", flush=True)
else:
    print(f"ERR verif {vc}: {verr}", flush=True)
