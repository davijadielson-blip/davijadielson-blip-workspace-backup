...
> Fonte: migração de `memory/decisions/` (8 arquivos) + decisões da migração v2.
...
**Decisão:** Emily não faz mais parte da equipe. Removida de todos os registros em `memory/`.
...
**Decisões:**
- Renomeada `[F2] claude/` → `memory/` (mais semântico, alinhado com o kit)
- CLAUDE.md movido para raiz do vault (carregamento prioritário pelo Claude Code)
- Adotada estrutura `memory/context/` com 5 arquivos de estado atual
...
- `CLAUDE_OBSIDIAN.md` → arquivado em `memory/decisions/`; CLAUDE.md único na raiz
...
**Contexto:** Briefings em `memory/agents/` dependiam de invocação manual. Subagents nativos em `.claude/agents/` permitem invocação por `@nome` ou automática.

**Decisão:** Manter dois sistemas coexistindo — briefings em `memory/agents/` (contexto rico) + subagents em `.claude/agents/` (invocação nativa).
...
**Decisão:** 22 comandos em `.claude/commands/` cobrindo rotina, legendas, posts por frente, agenda e manutenção. Todo output nasce em `memory/outputs/` — nunca direto na pasta da frente.
...
**Decisão:** Usar MCP nativo `mcp__claude_ai_Google_Calendar__*` para buscar eventos reais. Classificação por frente via `memory/databases/regras-classificacao-agenda.md`.
...
**Decisões tomadas:**
- Rotina-Diretor formalizada em `memory/context/rotina.md` (Inversão Biológica + tetos + Zero-Sum + Ancoragem)
- Matriz dos 5 Destinos criada com 37 tarefas categorizadas em `memory/databases/matriz-tarefas.md`
- Plano de 30 dias com despertar progressivo (06h → 04h30) criado em `memory/projects/plano-30-dias-diretor.md`
...
- 4 camadas de skills mapeadas em `memory/_MAP-skills.md` com gatilhos automáticos
- Projeto Regra 1/3 (patrimônio judaico) iniciado em `memory/projects/regra-1-3-patrimonio.md` — revisão trimestral
...
- `memory/projects/` criada como nova subpasta (não existia)
...
**Decisão:** Criar Hub central, Canvas geral, dashboards Dataview e diagramas Mermaid em `memory/visualizations/`.
...
**Arquivos auditados:**
- `memory/context/rotina.md` — 🟡 Despertar Progressivo atualizado (Semana 1 ✅ → Semana 2 atual); `revisado: true`
- `memory/databases/matriz-tarefas.md` — ✅ Correto; `revisado: true`
- `memory/projects/regra-1-3-patrimonio.md` — ✅ Correto (Sessão 1 aguarda Jadielson); `revisado: true`
- `memory/projects/plano-30-dias-diretor.md` — 🟡 `semana-atual: 1 → 2`; typo "treinsar" → "treinar"; `revisado: true`
- `memory/_MAP-skills.md` — 🟡 Nota stale `/mnt/skills/user/` substituída por nota de objetivo futuro; `revisado: true`

**Status:** ✅ Auditoria concluída — prontos para tag v3.5.0.

---

### [28/05] Convenção de nomes de arquivo — compatibilidade iOS/Windows/OneDrive

**Contexto:** Jadielson usa iOS e Windows. O OneDrive recodifica automaticamente caracteres inválidos no Windows, causando inconsistências nos nomes de arquivo.

**Regra permanente:** Nunca usar os seguintes caracteres em nomes de arquivo ou pasta:

```
?  "  *  :  <  >  \  |
```

**Motivo:** incompatibilidade com o sistema de arquivos NTFS (Windows) — o OneDrive substitui esses caracteres toda vez que o arquivo é acessado em Windows ou iOS, quebrando links e referências no vault.

**Aplica-se a:** todos os arquivos `.md`, pastas, scripts, outputs e qualquer item criado no vault.
